...
Return the generated image.`;
    ...
    // 상세페이지가 비어있으면 자동 구성 (에디터의 autoGenerateDetailPage와 유사)
    if (!product.detailPageItems || product.detailPageItems.length === 0) {
      console.log('[AI Auto Edit] 상세페이지 자동 구성 시작');
      ...
      if (userSettings.brandImageUrl) {
        product.detailPageItems.push({
          type: 'brand-image',
          imageSrc: userSettings.brandImageUrl,
          label: '브랜드 이미지'
        });
      }
      ...
      // 3. 제품명
      const productTitle = product.title || product.titleCn || '제품명';
      product.detailPageItems.push({
        type: 'title',
        text: productTitle,
        label: productTitle,
      ...
      if (product.results && product.results.length > 0) {
        ...
        option1Groups.forEach((group, opt1) => {
          const uniqueOption2 = [...new Set(group.option2List)].filter(v => v);
          product.detailPageItems.push({
            type: 'option-block',
            imageSrc: group.imageSrc,
            optionName: opt1,
            option2Text: uniqueOption2.join(', '),
            label: `옵션: ${opt1}`
          });
        });

        console.log(`[AI Auto Edit] 옵션 블록 ${option1Groups.size}개 추가`);
      }
      ...
      const qualityRows = [
        ...
        { label: '사이즈', value: '상세페이지 참조' },
        { label: '제조국', value: 'Made in China' },
        { label: '제조사', value: userSettings.qualityTable?.제조사 || '상세페이지 참조' },
        { label: '취급시 주의사항', value: userSettings.qualityTable?.취급시주의사항 || '상세페이지 참조' },
        { label: '제조 연월', value: userSettings.qualityTable?.제조연월 || '상세페이지 참조' },
        { label: '품질보증기준', value: '관련법 및 소비자 분쟁해결 규정에 따름' },
        { label: 'A/S 책임자와 전화번호', value: userSettings.qualityTable?.AS책임자 || '상세페이지 참조' }
      ];
      ...
      console.log(`[AI Auto Edit] 상세페이지 자동 구성 완료 (${product.detailPageItems.length}개 아이템)`);
